...
- 💾 **Current Card Face Export (experimental):** Export the existing Wallet display cache and extract its card-face image. This has produced a user-confirmed 1272 × 780 PNG on one iOS 27.0 device. Separate read-only and move-based methods support the combined PNG/PDF files used by Flash Skins; issuer-source artwork and general device compatibility are not guaranteed.
...
5. Click **Back Up Artwork** to choose an export method, or click **Flash Skins** and choose **Back Up First…** to open the same export options. Backup never automatically flashes a skin. Check the exported files, then start **Flash Skins** separately when ready. **Flash Without Backup** retains the previous flash workflow.
...
Connect the same unlocked, trusted iPhone used to detect the card hash. Click **Back Up Artwork** to choose an export method; no imported image is required. The current Wallet display can be stored in `.cache/FrontFace` even when the three `cardBackgroundCombined` files are absent. These are different sources, so a failed combined-file export does not establish that the current card-face image cannot be recovered. The export additions described here require this modified build; an upstream release may not include them.
...
Before exporting, turn off Express Mode for the selected card in Wallet, then open the card and scan it again. This resolved the reported export failure for one user; it is not a guarantee for every card or device. Keep the selected output folder in place until the operation finishes.

### Export the current displayed card face

Select exactly one disposable test card, click **Back Up Artwork**, and choose **Export Current Card Face…**. Confirm the exact card and choose an output folder. This exports its existing `/var/mobile/Library/Passes/Cards/<card_hash>.cache/FrontFace`, then extracts the archived `faceImage` bytes. It does not flash a replacement picture or invalidate Wallet caches, and it never proceeds to automatic flashing. If only a saved or bare identifier was observed, the app separately explains that it has not verified the full Wallet path.
...
### Export the combined PNG/PDF files

**Read-Only Files** and **Combined Files (Experimental)…** target `cardBackgroundCombined@3x.png`, `cardBackgroundCombined@2x.png`, and `cardBackgroundCombined.pdf`. A complete three-file backup requires all three files and verified cleanup.
...
### Explicit experimental move export — disposable test cards only

Click **Back Up Artwork**, choose **Combined Files (Experimental)…**, and select exactly one disposable card. A second warning names the exact card before any operation. This method does not need an imported image, does not invalidate Wallet caches, and never proceeds to automatic flashing. Do not use it on irreplaceable artwork yet. Keep the phone unlocked and connected, and do not use Wallet, Books, another Airlift tool, or a second AirCard operation while it runs.
